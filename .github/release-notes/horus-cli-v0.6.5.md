...
- Renders gateway-supplied queued active input at the transcript tail without capability-specific
  terminal dispatch.
- Bundles gateway protocol 20, `horus-gateway` 0.6.5, and `horus` 0.6.4.

## Install or upgrade

```sh
horus-gateway exit
cargo install --force --locked horus-cli --version 0.6.5
```
...
- No automatic migration is provided. Back up the gateway state directory before upgrading. If it
  is incompatible, run `horus-gateway init` and explicitly confirm reinitialization; this permanently
