...
- Bundles `mobius-gateway` 0.2.0 and `mobius` 0.2.0 with patch application, workspace instructions,
...
- Keeps the source package sliced to CLI code while the install/archive intentionally includes the
  sibling `mobius-gateway` executable used for automatic local startup.
