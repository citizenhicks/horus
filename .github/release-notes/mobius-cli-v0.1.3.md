...
- One install now provides both user-facing commands: `mobius` and `mobius-gateway`. GitHub CLI
  archives also contain both binaries; the core `mobius` 0.1.2 library is linked automatically.
...
- This client uses gateway protocol version 4 and requires `mobius-gateway` 0.1.2. Older gateways
...
If `mobius-gateway` was installed from its old standalone crate, transfer both commands once:
...
`~/.mobius/gateway` and `~/.mobius/gateway-tokens.json`), then run `mobius` to create and pair fresh
state.
