...
- Bundles `mobius` 0.6.4 and keeps capability policy in middleware rather than the agent loop.
...
- There is no automatic state migration. Back up the gateway state directory first; the supported
  fresh-start path is `mobius-gateway init`, whose explicit reinitialization confirmation permanently
