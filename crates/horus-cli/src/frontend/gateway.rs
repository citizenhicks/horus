...
impl State {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> horus_gateway::Result<Action> {
    ...
    fn handle_accounts_key(&mut self, key: KeyEvent) -> horus_gateway::Result<Action> {
        match key.code {
    ...
    fn handle_add_key(&mut self, key: KeyEvent) -> horus_gateway::Result<Action> {
        match key.code {
            KeyCode::Esc => {
    ...
    fn reconnect_selected(&mut self) -> horus_gateway::Result<Action> {
    ...
    fn forget_selected(&mut self) -> horus_gateway::Result<Action> {
        self.require_saved_management()?;
        let Some(endpoint) = self
            .accounts
            .endpoints()
    ...
    fn open_add(&mut self) -> horus_gateway::Result<Action> {
        self.require_saved_management()?;
        self.page = Page::Add;
    ...
    fn require_saved_management(&self) -> horus_gateway::Result<()> {
        if let Some(message) = self.override_message {
            return Err(horus_gateway::Error::Config(message.into()));
        }
        Ok(())
    }
    ...
    fn pairing_request(&self) -> horus_gateway::Result<(Endpoint, String)> {
...
fn render_accounts(lines: &mut Vec<Line<'static>>, state: &State) {
    ...
    if state.accounts.endpoints().len() == 0 {
        lines.push(Line::styled(
            "  None saved · Horus uses the automatic local gateway by default.",
