...
use horus::protocol::FrontendActiveInput;
use horus::protocol::FrontendContribution;
...
fn cli_commands() -> Vec<UiCommand> {
    vec![
        ...
        command("exit", "exit Horus", false, CommandHandler::Exit),
    ]
}
...
mod tests {
    use super::*;
    use horus::protocol::FrontendCommand;
    use horus::protocol::FrontendReference;
    ...
    async fn merges_middleware_provider_and_workspace_contributions() {
        ...
        let mut contribution = contribution("inspect");
        contribution.references.push(FrontendReference {
            trigger: '@',
