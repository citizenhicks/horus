...
use horus_gateway::client::{Endpoint, token_from_env};
...
pub fn environment_override_message() -> Option<&'static str> {
    match (
        ...
        (true, true) => Some(
            "Gateway selection is controlled by HORUS_GATEWAY_ENDPOINT and HORUS_GATEWAY_TOKEN. Unset them to manage saved gateways.",
        ),
        (true, false) => Some(
            "Gateway selection is controlled by HORUS_GATEWAY_ENDPOINT. Unset it to manage saved gateways.",
        ),
        (false, true) => Some(
            "Gateway selection is controlled by HORUS_GATEWAY_TOKEN. Unset it to manage saved gateways.",
        ),
        (false, false) => None,
    }
}
...
fn token_path() -> Result<PathBuf> {
    if let Some(path) = env::var_os("HORUS_GATEWAY_TOKEN_FILE") {
        return Ok(path.into());
    }
    env::var_os("HOME")
        ...
        .ok_or_else(|| {
            Error::Config("cannot determine token path; set HORUS_GATEWAY_TOKEN_FILE".into())
        })
}
